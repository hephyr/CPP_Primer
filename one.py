# -*- coding:utf-8 -*-
import urllib
import urllib2
import re
class One:
    def __init__(self) :
        self.page = 1
        self.cur_url = "http://wufazhuce.com/one/vol.{page}"
        self.datas = []
        print "loading..."
    def get_page(self, cur_page) :
        url = self.cur_url
        try :
            my_page = urllib2.urlopen(url.format(page = cur_page)).read().decode("utf-8")
        except urllib2.URLError, e :
            if hasattr(e, "code"):
                print "The server couldn't fulfill the request."
                print "Error code: %s" % e.code
            elif hasattr(e, "reason"):
                print "We failed to reach a server. Please check your url and read the Reason"
                print "Reason: %s" % e.reason
        return my_page
    def saveImg(self,my_page) :
        imageURL = "http://pic.yupoo.com/hanapp/F31uTdGL/custom.jpg"
        pattern = re.compile('class="corriente".*?alt>',re.S)
        item = re.findall(pattern,my_page)
        items = re.findall(imageURL, item)
        u = urllib.urlopen(pattern)
        data = u.read()
        fileName = "One" + "/" + str(self.page)+".jpg"
        f = open(fileName, 'wb')
        f.write(data)
        f.close()
    def start_pic(self) :
        """
        爬虫入口, 并控制爬虫抓取页面的范围
        """
        while self.page <= 20 :
            my_page = self.get_page(self.page)
            self.saveImg(my_page)
            self.page += 1
def main() :
    onepic = One()
    onepic.start_pic()
    print "END..."

if __name__ == '__main__':
    main()